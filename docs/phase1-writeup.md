...
The cleaned CSV carries two loss columns: `Estimated_Loss_EUR` in the euros of each fire's own year, and `Estimated_Loss_EUR_2025` in 2025 euros. This assumes real EUR/ha is constant over time (no exposure or value growth; the PRD's value-growth sensitivity covers that), and HICP is a general consumer index, not a construction-cost index, so reconstruction-cost inflation may differ.
