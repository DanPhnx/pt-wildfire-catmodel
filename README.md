...
models/            Fitted distribution parameters (frequency, severity) - Phase 2, not yet populated
...
2. **Distribution fitting** (not yet implemented): Poisson or Negative Binomial
   frequency model (annual counts are strongly overdispersed, see below),
   Lognormal body with a Generalised Pareto tail for severity,
   Anderson-Darling/QQ/AIC-BIC goodness-of-fit
...
via `nbconvert`, and reports which phases ran. A phase whose functions still
raise `NotImplementedError` is a stub and is skipped with a message rather
than run into a confusing traceback - today that is Phases 2-4. Run
`python main.py --phase 1` for a single phase, or open a notebook directly
in Jupyter for interactive work.
...
**Phases 2-4: not started.** Notebook skeletons exist with stub functions
(`NotImplementedError`); see notebook headers for phase-by-phase task
tracking and `docs/worklog.md` for what Phase 1 found that should shape
them (overdispersion, frequency-severity dependence, same-day fire
clustering).
