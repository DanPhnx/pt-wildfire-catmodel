...
wildfire_model.py  Shared logic used by more than one phase notebook (e.g. fire-day
                   event construction, the train/hold-out split)
data/raw/          Real per-fire EFFIS database (committed), live OWID/GWIS/HICP cross-check
                   snapshots, and the loss calibration and anchors tables
data/processed/    Cleaned, per-fire-event CSV, regenerated by 01_eda.ipynb, not versioned
models/            Fitted distribution parameters, committed (small, human-readable JSON) -
                   frequency done (Phase 2), severity pending
