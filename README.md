...
monte_carlo.py          Phase 3's simulation engine (frequency/severity/frailty
                        sampling, severity tail cap, risk metrics, convergence
                        check, diagnostic plots) - same "notebooks for EDA
                        only" pattern; 03_monte_carlo.ipynb imports and calls
                        this. Module built and smoke-tested; the calibrated
                        production run is not yet done (see Status below and
...
2. **Distribution fitting** (done): events are grouped into fire-day clusters
   ...
   parameter.
3. **Monte Carlo simulation** (in progress): engine built in `monte_carlo.py` -
   Negative Binomial frequency, Lognormal/GPD severity (capped: the fitted GPD
   tail has infinite theoretical variance), a year-level frailty factor for
   the residual frequency-severity dependence Phase 1 found, VaR(90/95/99),
   Expected Shortfall(95), aggregate 1-in-10/25/100 return-period losses.
   100,000-scenario production run and frailty calibration not yet done - see
   `docs/phase3-frequency-severity-dependence.md`
...
**Phase 3 (Monte Carlo simulation): in progress.** `monte_carlo.py` is built
and smoke-tested (frequency/severity sampling, the year-level frailty factor,
risk metrics, a convergence check, diagnostic plots), per the design in
`docs/phase3-frequency-severity-dependence.md`. Not yet done: the fitted GPD
tail (xi=0.746) has infinite theoretical variance, so every severity draw is
capped (a practical bound and an ICNF-sourced physical ceiling); a joint
calibration of the frailty factor's two parameters against both the observed
annual variance and the observed count/severity correlation, checked against
the 2021-2025 hold-out; the actual 100,000-scenario production run; and the
euro-loss conversion and results CSV the PRD asks for. `03_monte_carlo.ipynb`
loads Phase 2's fitted parameters and calls the module but leaves the run
itself commented out until that calibration is complete.

**Phase 4 (validation and sensitivity): not started.** Notebook skeleton
exists with stub functions (`NotImplementedError`, not yet called); see the
notebook header for task tracking and `docs/worklog.md` for what Phase 1-3
found that should shape it (overdispersion, frequency-severity dependence,
